 ...
 * an `undefined` value.
 */
export function makeSchemaParser<T extends z.ZodTypeAny>(schema: T): (input: unknown) => z.infer<T>
export function makeSchemaParser<T extends z.ZodTypeAny>(schema: T, strict = true): (input: unknown) => z.infer<T> | undefined {
  if (strict) {

...
const partSchema = z.string().or(
  z.object({ name: z.string(), order: z.string().optional() })
...
export const CreativeWorkSchema = ThingSchema.extend({
  ...
  creator: z.string().or(z.record(z.string().or(z.array(z.string())))).optional(), // Either a single string, or a dictionary of strings or string arrays.

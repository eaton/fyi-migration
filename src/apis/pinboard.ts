...
export class Pinboard {
  ...
  async lastUpdate(force = false) {
    if (!this._lastUpdate || force) {
      this._lastUpdate = await this.api
        .get(this._endpoint + '/posts/update')
        .json()
        .then(
          json =>
            z.object({ update_time: z.coerce.date() }).parse(json).update_time,
        );
    }
  ...
  async getAll(options: PinboardPostFilter = {}): Promise<PinboardLink[]> {
    ...
    return this.api
      ...
      .error(429, () => {
        throw new Error('Pinboard rate limit exceeded');
      })
  ...
  async listNotes() {
    return await this.api
      .get('https://api.pinboard.in/v1/notes/list')
      .json(json =>
        z.object({ count: z.number(), notes: z.array(noteSchema) }).parse(json),
      );
  }

  async getNote(id: string) {
    return await this.api
      .get(`https://api.pinboard.in/v1/notes/list/${id}`)

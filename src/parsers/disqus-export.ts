...
export async function parse(xml: string | Buffer, options: Options = {}) {
  const extracted = await extract(xml ?? '', DisqusXmlTemplate, DisqusXmlSchema, { xml: true });
...
const postSchema = z.object({
  ...
  forum: z.string().optional().describe("Not present in native export; optionally populated with the forum name."),
  category: z.string().optional().describe("Not present in native export; optionally populated with the category ID of the post's thread."),
  thread: z.coerce.string(),
  parent: z.coerce.string().optional(),
  sort: z.string().optional().describe("Not present in native export; optionally populated with a thread-aware sorting key."),
  createdAt: z.coerce.date(),
  isSpam: z.unknown(),
  isDeleted: z.unknown(),
  link: z.string().optional().describe("Not present in native export. Optionally populated with the link the thread replies to."),
  linkId: z.string().optional().describe("Not present in native export. Optionally populated with the ID of the link the thread replies to."),
  author: authorSchema,
  message: z.string(),
}).passthrough();
...
const threadSchema = z.object({
  ...
  isClosed: z.unknown(),
  isDeleted: z.unknown(),
  posts: z.array(postSchema).default([]).describe("Not present in native export; optionally populated with all posts in a thread."),
}).passthrough();
...
const categorySchema = z.object({
  ...
  title: z.string(),
  isDefault: z.unknown(),
  threads: z.array(threadSchema).default([]).describe("Not present in native export; optionally populated with all threads in a category."),
});
